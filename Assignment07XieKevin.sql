...
-- 2021-5-20, Kevin Xie,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_XieKevin')
	 Begin 
	  Alter Database [Assignment07DB_XieKevin] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_XieKevin;
	 End
	Create Database Assignment07DB_XieKevin;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_XieKevin;
...
-- <Put Your Code Here> --
Create 
View vProductNameByPrice
As
Select 
 ProductName,
 format (UnitPrice, 'C', 'en-US') as [US Price] 
from Products;
go
Select * from vProductNameByPrice order by ProductName
go
...
AS 
  ...
  C.CategoryName,
  P.ProductName, 
  [US Price] = format (P.UnitPrice, 'C', 'en-US')
...
From Products as P
 ...
 on P.ProductID = I.ProductID ;
Go
 Select * from vProductInventories Order By InventoryDate,ProductName,[Count];
 Go
...
Go
Select * from vCategoryInventories Order by CategoryName, InventoryDate;
Go
...
as
 ...
 [Previous Month Count] = isnull(lag([count]) over (order by InventoryDate),0)
 from vProductInventories;
 Go
 Select * from vProductInventoriesWithPreviouMonthCounts order by ProductName, InventoryDate
go
...
Go
Select * From vProductInventoriesWithPreviousMonthCountsWithKPIs order by ProductName, InventoryDate,[Inventory count];
go
...
As
Return
  Select top 100000 * from dbo.vProductInventoriesWithPreviousMonthCountsWithKPIs
where [KPI] = @kpi
